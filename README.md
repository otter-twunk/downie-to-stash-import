...
This creates a fresh `.venv-build`, installs the package plus PyInstaller, runs `packaging/downie_stash.spec`, and outputs:
...
2. Run this tool to generate `scenes/` plus the report files.
3. Use Stash's JSON import task with the generated bundle.
...
- **Low match rates**
  - Check filename and title naming consistency.
  - Adjust `--min-score` if the matcher is being too strict.
- **Ambiguous matches**
  - Inspect `ambiguous.json`.
  - Rename media files or remove duplicates to improve confidence.
- **No media indexed**
