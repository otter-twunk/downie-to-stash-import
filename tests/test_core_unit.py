...
def test_normalize_text_handles_url() -> None:
    assert normalize_text("https://example.com/Alpha Scene") == "example com alpha scene"


def test_clean_title_strips_boilerplate() -> None:
    value = "My Video - | JustTheGays - justthegays.tv"
    assert clean_title(value) == "My Video"
...
def test_score_candidate_zero_for_mismatch() -> None:
    score, _ = score_candidate(_downie(stem="zzzz", title="zzzz"), _media(stem="alpha scene"))
    assert score < 20
