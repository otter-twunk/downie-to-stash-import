...
class App(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Entry(json_frame, textvariable=self.json_root_var, width=60).grid(
            row=0,
            column=1,
            sticky="we",
            padx=4,
        ...
        ttk.Button(media_frame, text="Remove selected", command=self._remove_media_root).grid(
            row=1,
            column=1,
            sticky="ew",
            pady=2,
        ...
        ttk.Entry(out_frame, textvariable=self.output_root_var, width=60).grid(
            row=0,
            column=1,
            sticky="we",
            padx=4,
        ...
        ttk.Label(opt_frame, text="Details text:").grid(row=0, column=1, sticky="e", padx=(20, 4))
        ...
        ttk.Label(opt_frame, text="Min score:").grid(row=1, column=1, sticky="e", padx=(20, 4))
        ttk.Spinbox(
            opt_frame,
            from_=0,
            to=200,
        ...
        self.log_text = tk.Text(run_frame, height=14, wrap="word")
        self.log_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scroll = ttk.Scrollbar(run_frame, orient="vertical", command=self.log_text.yview)
        scroll.pack(side=tk.RIGHT, fill=tk.Y)
        self.log_text.configure(yscrollcommand=scroll.set)

        summary_frame = ttk.LabelFrame(main, text="Results summary")
        summary_frame.pack(fill=tk.X, pady=(8, 0))
        ttk.Label(summary_frame, text="Matched count:").grid(row=0, column=0, sticky="w")
        ttk.Label(summary_frame, textvariable=self.summary_matched_var).grid(row=0, column=1, sticky="w", padx=(4, 20))
        ttk.Label(summary_frame, text="Unmatched count:").grid(row=0, column=2, sticky="w")
        ttk.Label(summary_frame, textvariable=self.summary_unmatched_var).grid(row=0, column=3, sticky="w", padx=(4, 20))
        ttk.Label(summary_frame, text="Ambiguous count:").grid(row=0, column=4, sticky="w")
        ttk.Label(summary_frame, textvariable=self.summary_ambiguous_var).grid(row=0, column=5, sticky="w", padx=(4, 0))
        ttk.Label(summary_frame, text="Media files indexed:").grid(row=1, column=0, sticky="w", pady=(4, 0))
        ttk.Label(summary_frame, textvariable=self.summary_media_var).grid(row=1, column=1, sticky="w", padx=(4, 20), pady=(4, 0))
        ttk.Label(summary_frame, text="Output folder:").grid(row=1, column=2, sticky="w", pady=(4, 0))
        ttk.Label(
            summary_frame,
            textvariable=self.summary_output_var,
        ).grid(row=1, column=3, columnspan=3, sticky="w", padx=(4, 0), pady=(4, 0))
    ...
    def _choose_output_root(self) -> None:
        path = filedialog.askdirectory(title="Select output folder (Stash import bundle)")
        if path:
    ...
    def _remove_media_root(self) -> None:
        selected = list(cast(tuple[int, ...], self.media_list.curselection()))  # type: ignore[no-untyped-call]
        if not selected:
            return
    ...
    def _collect_config(self) -> ConversionConfig | None:
        ...
        try:
            ...
            messagebox.showerror("Error", "Min score and ambiguity gap must be numbers.")
            return None
    ...
    def _update_summary(self, summary: dict[str, object]) -> None:
        ...
        self.summary_output_var.set(str(summary.get("output_root", self.output_root_var.get())))
    ...
    def _on_run_clicked(self) -> None:
        ...
        def worker() -> None:
            ...
            try:
                ...
                summary_with_output["output_root"] = str(config.output_root.resolve())
                self.after(0, self._on_run_done, None, summary_with_output, config.dry_run)
    ...
    def _on_run_done(
        ...
        if summary is None:
            self.status_var.set("Error")
            self.open_output_button.config(state=tk.DISABLED)
            messagebox.showerror("Error", "Conversion finished without a summary.")
            return
        ...
        messagebox.showinfo("Done", "Conversion complete. Check the log and output folder.")
    ...
    def _open_output_folder(self) -> None:
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to open output folder: {error}")
