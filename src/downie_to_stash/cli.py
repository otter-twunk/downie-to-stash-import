#!/usr/bin/env python3
"""CLI entrypoint for Downie to Stash conversion."""
...
from downie_to_stash import __version__
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--min-score", type=float, default=60.0, help="Minimum score to accept a match."
    )
    parser.add_argument(
        "--ambiguity-gap",
        type=float,
    ...
    parser.add_argument(
        "--allow-stream-url",
        action="store_true",
        help="Use Downie media URL if no referer or source page URL exists.",
    )
    parser.add_argument(
        "--no-date",
        action="store_true",
        help="Do not map Downie timestamps to scene date.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def _render_summary_table(summary: dict[str, object]) -> str:
    ...
    separator = f"├{'─' * (metric_width + 2)}┼{'─' * (count_width + 2)}┤"
    lines = [
        f"│ {label.ljust(metric_width)} │ {str(count).rjust(count_width)} │"
        for label, count in rows
    ]
    bottom = f"└{'─' * (metric_width + 2)}┴{'─' * (count_width + 2)}┘"
    return "\n".join([top, header, separator, *lines, bottom])
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    def log(message: str) -> None:
        if args.verbose or any(
            token in message for token in ("MATCH", "SKIP", "Done", "Error")
        ):
    ...
    has_issues = (
        ...
        or _as_int(summary.get("ambiguous_count", 0)) > 0
    )
